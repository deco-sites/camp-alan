...
import { clx } from "deco-sites/camp-alan/sdk/clx.ts";
import { ProductCardFlagProps } from "deco-sites/camp-alan/flags/multivariate.ts";
...
export function ErrorFallback({ error: _error, ...props }: { error?: Error }) {
  return (
    <div class="flex flex-col mx-auto max-w-96 gap-3">
      <img
        src={asset("/image/error.jpg")}
        alt={"moqueca"}
        height={400}
        width={400}
      />
      <p class={"font-bold "}>Festa do congo</p>
      <p>
        A grande festa movimenta a região de Roda D’Água, zona rural de
        Cariacica, anualmente, no mesmo dia da festa da padroeira do Espírito
        Santo, Nossa Senhora da Penha, e conta com uma programação extensa que
        se inicia pela manhã, com a saída da casa do “festeiro”, que recebe em
        sua casa os congueiros e segue com o cortejo em direção ao local da
        missa campal.
      </p>

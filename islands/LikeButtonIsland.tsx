...
import Icon from "deco-sites/camp-alan/components/ui/Icon.tsx";
import { invoke } from "deco-sites/camp-alan/runtime.ts";
import { total } from "deco-sites/camp-alan/sdk/useTotalLikes.ts";
...
function LikeButtonIsland({ productID }: LikeButtonIslandProps) {
  ...
  useEffect(() => {
    const updateTotals = async () => {
      const totalLikes = await invoke["deco-sites/camp-alan"].loaders
        .totalLikesLoader();
      const totalLikesProduct = await invoke["deco-sites/camp-alan"]
        .loaders
  ...
  const handleToggleLike = async (e: MouseEvent) => {
    e.preventDefault();
    const result = await invoke["deco-sites/camp-alan"].actions
